...
起動時はADC1/ADC2の自己校正後、モーターPWM出力をOFFにしたまま1000組を取得し、
...
0,1,0.125,3.250,-0.500,1.000
1,1,-0.875,2.250,-1.500,0.000
```

- `sample`は0～3999の取得順番号です。
- `sector`は120度駆動中の通電ステップ番号1～6です。停止中および従来の
  手動PWMモードでは0です。
- `u1_delta`はADC1 Rank 1（VOPAMP1）、`v_delta`はADC2 Rank 1（VOPAMP2）、
  `u2_delta`はADC1 Rank 2（VOPAMP1）、`w_delta`はADC2 Rank 2
  （VOPAMP3）の「12bit raw値 − 対応する起動時平均」です。
  単位はADCカウント（A換算ではありません）。負値を含み、小数点以下3桁で出力します。
  平均の小数部分を保持するための表示であり、ADCの分解能が増えるわけではありません。
  従来の`*_raw`列から列名・値の意味が変わるため、CSV読み取り側も対応が必要です。
- `u1_delta`/`v_delta`は同時に変換され、その直後に`u2_delta`/`w_delta`が
  同時に変換されます。
